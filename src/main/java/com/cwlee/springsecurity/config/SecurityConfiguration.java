...
//                .permitAll());                                                                      // /product/[숫자] 로 이루어진 요청만 허용
//        httpSecurity.authorizeHttpRequests(c -> c.anyRequest().denyAll());                          // 그 외에는 모두 거부

        httpSecurity.authorizeHttpRequests(c -> c.requestMatchers(HttpMethod.GET, "/api/v1/hello")
                .authenticated());                                                                  // GET방식으로 /api/v1/hello 를 요청하면 앱이 사용자를 인증해야 함
        httpSecurity.authorizeHttpRequests(c -> c.requestMatchers(HttpMethod.POST, "/api/v1/hello")
                .permitAll());                                                                      // POST방식으로 /api/v1/hello 를 요청하면 모두 허용함
        ...
        httpSecurity.csrf(AbstractHttpConfigurer::disable);                                         // HTTP POST 방식으로 /aoi/h1/hello를 호출할 수 있도록 CSRF 비활성화
